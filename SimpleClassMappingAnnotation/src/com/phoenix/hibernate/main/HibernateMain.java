...
public class HibernateMain {
	...
	public static void main(String[] args) {
		...
		//CRUD operations
		/*
		 * Product product = ss.get(Product.class, 104); System.out.println(product);
		 */
		
		//inserting a row by object
		/*
		 * Product p = new Product(110,"Mobile","Samsung",50000.00f); ss.save(p);
		 */
		...
		/*
		 * Product p = ss.get(Product.class, 110); if (p != null) {
		 * p.setPrice(53000.00f); ss.update(p); } else {
		 * System.out.println("Sorry!! Product not found"); } p = ss.get(Product.class,
		 * 110); if (p != null) System.out.println(p); else
		...
		/*
		 * Product p = new Product(); p.setProductName("Desktop Computer");
		 * p.setBrand("Dell"); p.setPrice(63000.00f); ss.save(p);
		 */
